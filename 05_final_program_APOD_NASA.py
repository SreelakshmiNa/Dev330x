...
def parse_command_line():
    ...
    parser.add_argument('-d', '--date', nargs = 3, metavar = ("month", "day", "year"), default = (), type = int, help = 'formatted date (i.e. 03 28 1998)')
    parser.add_argument('-s', '--surprise', action = 'store_true', help = 'select a random date for a surprise image')
    parser.add_argument('-k', '--api_key', nargs = 1, metavar = ("API_KEY"), default = '', help = 'NASA developer key')
...
def create_date(datelist, surprise):
    ...
    # today
    now = datetime.today()
    today =  date(month=now.month, day=now.day, year=now.year)
    # yesterday
    one_day = timedelta(days = 1)
    yesterday = today - one_day
    d = yesterday
    # min date
    min_date = date(month=6, day=16, year=1995)
    
    try:
        if len(datelist) > 0:
            d = date(month=datelist[0], day=datelist[1], year=datelist[2])
        elif len(datelist) < 1 and surprise:
            while True:
                month = randint(1, 12)
                day = randint(1, 31)
                year = randint(1995, 2000)
                try:
                    d = date(month = month, day = day, year = year)
                    if d <= today and d >= min_date:
                        break
                except:
                    pass
        elif len(datelist) < 1 and not surprise:
            d = yesterday
        else:
            pass
    except:
...
def query_url(d, api_key):
    ...
    url = "https://api.nasa.gov/planetary/apod?api_key={:}&date={:}".format(api_key,d.strftime("%Y-%m-%d"))
...
def save_image(d, image):
    ...
    if not os.path.exists(year):
        os.mkdir(year)
    month = "{:}".format(d.strftime("%m"))
    if not os.path.exists(year+'/'+month):
        os.mkdir(year+'/'+month)
    file_path = "{:}/{:}/{:}.jpg".format(year, month,d)
    error = False
    try:
